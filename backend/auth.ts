...
export function setupAuth(app: Express) {
    ...
    app.post("/api/auth/register", async (req: Request, res: Response) => {
        try {
            ...
            const user = await storage.createUser({ username, password: hashedPassword, fullName: fullName || null });

            req.login(user, (err) => {
                if (err) {
                    return res.status(500).json({ error: "Failed to create session" });
                }
                return res.status(201).json({ id: user.id, username: user.username, fullName: user.fullName });
            });
        } catch (error) {
